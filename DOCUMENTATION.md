...
Since the primary microcontroller in use is an STM32,
it's natural to use the vendor-provided application *STM32CubeIDE* to do the programming.
I, however, have huge disdain for that piece of software, summarized as so:

1. **It's annoying to install.**
The IDE is blocked behand a log-in wall and requires you to log into your ST account
(that you're forced to make) in order to use the IDE fully.
The installation is slow and is several gigabytes.
It's bloated with features we'd never use, so it's a waste of time and space.

2. **It's annoying to use.**
It takes several seconds to boot up.
The overall UX is poor.
There are a lot of strange graphical bugs in the UI.
There is a lot of buffering and loading.
There's frequent notifications about having to do updates and downloads.
Customizing the development experience within STM32CubeIDE is futile.

3. **It's annoying to version control.**
Creating a single STM32CubeIDE project results in dozens of files being created
that then have to be managed by Git.
Most of these files are redundant when there's multiple STM32CubeIDE projects
that are all based on the same MCU.
Some of these files are incomprehensible because the data within it is proprietary,
such as the XML files that describes the settings of the IDE.
The XML files are particularly painful
because it makes merge conflicts practically unresolvable if it ever so happens.
Overall, there just ends up being a lot of noise in the commit tree.

4. **It's pedagogically useless.**
The IDE does a lot of the heavy lifting when it comes to configuring the MCU
and getting things working,
but learning to use the IDE effectively is not *deep knowledge*.
Take this [tutorial](https://01001000.xyz/2020-08-09-Tutorial-STM32CubeIDE-SD-card/)
on how to get an uSD card module working in STM32CubeIDE
as an example.
It's just clicking and pressing buttons and dialog boxes,
copying lines and dragging files here and there.
If you're lucky, you'll end up with a working interface,
but if not, you're out of luck.

<p align="center">
<kbd>
<img src="./misc/media/stm32cubeide_gradient.png" height="400px">
<br>
<br>
<em>
Ugly gradients of STM32CubeIDE.
</em>
<br>
<br>
</p>&nbsp;

There's also *Visual Studio Code*
with the *STM32CubeIDE for Visual Studio Code* extension,
but it's not that much better in terms of addressing the issues above.
Visual Studio Code is certainly much more customizable than STM32CubeIDE,
but it has its own heap of other issues that makes it suboptimal for software development
for a project of this complexity on such a tight deadline.

Furthermore,
both Visual Studio Code and STM32CubeMX use *CMake* as their build system,
but really all *CMake* would do here is auto-generate *Make* files,
and it is these Make files that would actually specify how the source code should be compiled and linked.
However, Make is an ancient and limited tool,
and software design has significantly matured overall since then.
Thus both CMake and Make are nothing
but poorly designed build tools,
despite their entire purpose being to do that well.

So to have a productive development cycle,
the traditional tools of STM32CubeIDE, CMake, and Make have to be ditched.
Our custom development environment is instead centered around a "single" Python file
called `cli.py`.
It is from this Python script
that we can do things like building the source code,
flashing onto an MCU,
debugging an MCU, and much more.

<p align="center">
<kbd>
<img src="./misc/media/clipy_help.png" height="800px">
<br>
<br>
<em>
Help message from the Python script.
</em>
<br>
<br>
</p>&nbsp;

And this ends up being the right move for the fact that:

1. **It's trivial to set up.**
The act of cloning the RSXVT2026 repository
automatically "downloads" the custom development environment,
because the `cli.py` script is just like any other file that's tracked by Git.
Any other additional installations are then just done
through the usage of the `cli.py` script.
For instance,
if the user is attempting to build the source code,
but `arm-none-eabi-gcc` is missing,
the `cli.py` script will provide a diagnostic to the user about this
and how they can install the STM32 toolchain.

<p align="center">
<kbd>
<img src="./misc/media/clipy_missing_toolchain.png" height="200px">
<br>
<br>
<em>
The Python script indicates to the user about the missing command line tools.
</em>
<br>
<br>
</p>&nbsp;
...
2. **It's straight-forward to customize.**
All features of the `cli.py` script is entirely written in Python or through Python libraries.
This makes adding new commands and features to handle automation of tasks and data processing
proportional to one's familiarity with Python,
a very well-known and popular programming language.
This is in stark contrast to STM32CubeIDE's ugly ties
to the Eclipse IDE (which was originally designed for Java)
and Visual Studio Code's janky ways with JSON files.

3. **It's easy to version control.**
As stated already,
the `cli.py` script and other supporting Python files
are all tracked by Git,
just like any other source code in the repository.
Furthermore, because the `cli.py` can be modified to do anything we need it to do,
we can optimize it to make our code base as slim as possible;
no more needless duplications of source code.
This is discussed more in detail in the [Build System](#build-system) section.

4. **The tool is actually useful.**
By writing a Python script to handle compilation, flashing, debugging, and much more,
one learns the ins and outs of the involved tools at hand.
A crucial thing to also understand about the `cli.py` script is
that it serves well as an abstraction layer;
one does not need to concern themselves with how it works,
but in the event that it needs to be done,
the `cli.py` is completely open-source for scrutiny
(in Python no less).
The same cannot be said about STM32CubeIDE
when things suddenly break for inexplicable reasons
and there's not much you can do to figure out what's going wrong.

The decision to *not* use STM32CubeIDE is likely what has guaranteed us
our ability to program and debug our flight firmware on such a tight deadline.
STM32CubeIDE would've given us a good head start,
but we would've been incredibly limited by its constraints and issues later down the road.
The path to implementing an entire development environment from scratch
is not any easier either,
but it at least does not cost one's sanity to get the basic things done.

While there is still a lot that could be improved upon about the `cli.py` script,
there's not one thing I'd do differently about the development environment.
