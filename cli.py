...
@main_interface.new_verb(
    ...
    {
        'name'        : 'file_path',
        'description' : 'Path to the generatd log file data to verify.',
        'type'        : str,
    },
)
def verifyLogs(parameters):
    ...
    if not file_path.is_file():
        pxd.pxd_logger.error(f"Couldn't find file {repr(file_path.as_posix())}.")
        sys.exit(1)
    ...
    if file_index is None or file_path.suffix != '.log':
        pxd.pxd_logger.error(f"File {repr(file_path.as_posix())} doesn't seem like a log file.")
        sys.exit(1)
    ...
    while True:
        ...
        pxd.pxd_logger.info(
            f'Processing cluster {cluster_index} '
            f'({cluster_index * cluster_size / file_size * 100.0 :.2f}%)...'
        )
        ...
        if cluster_data != bytes((
            dumb_hash(file_index, cluster_index, sector_index, byte_index)
