...
@main_interface.new_verb(
    {
        'description' : 'Verify log file data generated by DemoSDMMC.',
    },
    {
        'name'        : 'path',
...
def verifyLogs(parameters):



    # @/`Dumb hash for verifyLog`:

    def dumb_hash(a, b, c, d):
        return ((((((((a * 0x9E37_79B1) ^ b) * 0x9E37_79B1) ^ c) * 0x9E37_79B1) ^ d) * 0x9E37_79B1) >> 24) & 0xFF



    # @/`File Name for verifyLog`:
    ...
    # Routine to verify a single file.

    def process_single_log_file(file_index, file_count, file_path):
        ...
        while True:
            ...
            pxd.pxd_logger.info(
                f'Processing {repr(file_path.as_posix())} ({repr(file_index + 1)}/{repr(file_count)}), '
                f'cluster {cluster_index} ({cluster_index * cluster_size / file_size * 100.0 :.2f}%)...'
            )
            ...
            if len(cluster_data) != cluster_size:
                pxd.pxd_logger.warning(
                    f'File size ({file_size :_} bytes) '
                    f'not multiple of expected cluster size ({cluster_size :_} bytes); '
                    f'remainder of {len(cluster_data) :_}'
                )
                sys.exit(1)
            ...
            if cluster_data != bytes((
                dumb_hash(file_number, cluster_index, sector_index, byte_index)
                for sector_index in range(cluster_size // 512)
                for byte_index   in range(512)
            )):
                pxd.pxd_logger.warning(f'Data mismatch at cluster index {cluster_index}!')
                sys.exit(1)
    ...
    if path.is_file():
        ...
        if get_file_number(path) is None:
            pxd.pxd_logger.error(f"File {repr(path.as_posix())} doesn't seem like a log file.")
            sys.exit(1)

        process_single_log_file(0, 1, path)
        ...
        files = sorted([
            ...
            if file.is_file()
            if get_file_number(file) is not None
        ])

        if files:
            for file_i, file in enumerate(files):
                process_single_log_file(file_i, len(files), file)
        else:
            pxd.pxd_logger.error(f"No log files found in {repr(path.as_posix())}.")
            sys.exit(1)

    else:

        pxd.pxd_logger.error(f"Path {repr(path.as_posix())} doesn't exist.")
        sys.exit(1)
...
def parseFlight(parameters):
    ...
    class LoRaPacket(ctypes.Structure):
        _fields_ = [
            ...
            ('timestamp_ms'              , ctypes.c_uint16),
            ('rolling_sequence_number'   , ctypes.c_uint16),
    ...
    class ESP32Packet(ctypes.Structure):
        _fields_ = [
            ...
            ('MagY'                 , ctypes.c_float      ),
            ('MagZ'                 , ctypes.c_float      ),
            ('image_sequence_number', ctypes.c_uint16     ),
    ...
    log_entry = LogEntry.from_buffer_copy(sector)
