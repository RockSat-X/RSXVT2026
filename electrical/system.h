...
// Ring-buffers.
//
// These ring-buffers is designed for
// single-producer, single-consumer situations.
// The length of the ring-buffers must be a power-of-two
// as an optimization requirement.
//
...
)
{
    ...
    if (ring_buffer_raw)
    {
        ...
        got_space = (u16) (observed_writer - observed_reader) < element_count;
        ...
        if (got_space && src)
        {
            ...
            memmove
            (
                ring_buffer_raw->bytes + index * element_size,
                src,
                element_size
            );
...
)
{
    ...
    if (ring_buffer_raw)
    {
        ...
        got_element = observed_reader != observed_writer;
